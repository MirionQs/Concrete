...
namespace concrete {
	...
	private:
		...
		const value_type _g;
		::std::vector<value_type> _root;

		void _precomputeRoots(::std::size_t size) noexcept {
			::std::size_t oldSize{_root.size()};
			if (size <= oldSize) {
				return;
			}
			...
			value_type n{_mod.modulo() >> (::std::countr_zero(oldSize) + 1)};
			for (::std::size_t half{oldSize}; half != size; half <<= 1, n >>= 1) {
				value_type r{_mod.power(_mod.inverse(_g), n)};
				for (::std::size_t i{half}; i != half << 1; i += 2) {
					_root[i] = _root[i >> 1];
					_root[i + 1] = _mod.multiply(_root[i], r);
				}
			}
		}
	...
	public:
		...
		void inverse_fast_fourier_transform(R&& range) noexcept {
			...
			for (::std::size_t i{0}; i != size; i += 2) {
				...
				x[i + 1] = _mod.multiply(_mod.subtract(p, q), inverse);
			}
